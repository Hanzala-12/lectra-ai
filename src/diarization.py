...
class SpeakerDiarization:
    ...
    def __init__(
        self, min_speakers: int = 1, max_speakers: int = 10, device: str = None
    ):
        """
        Args:
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.info(
                "Loading speaker diarization model (downloads ~700MB on first use)..."
            )
            ...
            _snapshot_dir = (
                models_dir / "models--pyannote--speaker-diarization-3.1" / "snapshots"
            )
            ...
            logger.warning(
                "Diarization will be disabled. Set HUGGING_FACE_HUB_TOKEN to enable."
            )
            self.pipeline = None
    ...
    def diarize(self, audio_path: str) -> List[Dict]:
        ...
        try:
            ...
            logger.info(
                f"Diarization complete: {n_speakers} speakers, {len(segments)} segments"
            )
