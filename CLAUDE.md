...
Every GPU-capable stage (DeepFilterNet, diarization, MetricGAN+, faster-whisper) auto-detects `cuda` vs `cpu` on its own — nothing to configure locally. There's no local GPU in this project's normal setup, so this always resolves to `cpu` unless a `GPU_TUNNEL_URL` is configured.

### GPU offload (optional, `gpu_tunnel/`)
`backend.py`'s `initialize_pipeline()` returns a `RemoteGpuPipeline` (`src/remote_pipeline.py`) instead of a local `LectraAIPipeline` when `GPU_TUNNEL_URL` is set, POSTing the audio file to a Kaggle-notebook-hosted worker (`gpu_tunnel/worker_app.py`, reached over a `cloudflared` tunnel — see `gpu_tunnel/README.md`) instead of processing it on CPU. `process_audio()` catches any failure from that call and falls back to a local `LectraAIPipeline` for that request (lazily built once, cached in `_local_fallback_pipeline`) — a missing/unreachable/dead tunnel degrades to local CPU processing, same "optional, doesn't crash" pattern as every other env-var-gated feature in this file. Video files never go over the tunnel (audio only); check `GET /api/health`'s `gpu_tunnel` field for whether one's configured/reachable.
...
- `backend.py` runs uvicorn without `reload=True` and loads the pipeline/models once at startup — **restart the process after changing `backend.py` or anything under `src/`**; it will not pick up changes live. Same for `GPU_TUNNEL_URL`/`GPU_TUNNEL_TOKEN` — set in `.env`, only read at startup.
- On the Kaggle side of `gpu_tunnel/`, never add `torch`/`torchaudio`/`numpy`/`scipy` to `kaggle_requirements.txt` — Kaggle's notebook image ships those pre-installed with CUDA support matched to its own GPU, and pip-installing the pinned CPU-targeted versions from `requirements-prod.txt` would silently replace them with a CPU-only build (everything still "works", just slow, with no obvious error).
...
- **Before claiming "the frontend"/"the backend" is running, built, or fixed — verify which checkout you actually touched.** Claude Code sessions on this repo often run inside an auto-created git worktree under `.claude/worktrees/<name>/` — a separate, independently-stale checkout (its own `frontend/`, `backend.py`, models, even its own `CLAUDE.md`) that silently diverges from `D:\fyp` (the real one) as master moves on. Run `git worktree list` if anything about the app's state looks inconsistent with recent work — a worktree frozen many commits behind is the first thing to suspect, not a bug in the app. Any tool that starts a dev server by resolving a path relative to *its own* working directory (rather than one you pass explicitly) will silently serve the worktree instead of `D:\fyp` — don't use one for this project. Start `npm run dev` / `python backend.py` via an explicit absolute path instead, and confirm which directory is actually being served before reporting anything as working (Windows: `Get-CimInstance Win32_Process -Filter "Name='node.exe'" | Select-Object ProcessId, CommandLine`).
- **Always run this project's own venv (`D:\fyp\venv\Scripts\python.exe` / `pip.exe` on Windows) — never bare `python`/`pip`.** This machine also has an unrelated system-wide Python on PATH with none of this project's ML dependencies (torch/torchaudio/deepfilternet/pyannote.audio/speechbrain), which plain `python`/`pip` resolve to instead of the venv. Testing against the wrong one produces a convincing but false "this dependency was never installed" conclusion — activate the venv (or use its full `Scripts\` path) before running anything, every time.
- **`GPU_TUNNEL_URL` in `.env` changes every time `gpu_tunnel/kaggle_worker.ipynb` is restarted** (Kaggle's free `cloudflared` quick-tunnel has no persistent domain — a fresh URL prints each session). Update `.env` and restart the backend after every restart of that notebook; a dead/unreachable tunnel URL usually just means it's expired, not that something's broken — see `gpu_tunnel/README.md`.
