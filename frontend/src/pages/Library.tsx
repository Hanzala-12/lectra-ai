...
export function Library() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-8 sm:px-10 py-10 md:pl-6">
      <Reveal className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between mb-8">
        <div>
      ...
      <Reveal delay={0.08} className="relative mb-9 max-w-sm">
        ...
        <StaggerGroup className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((l) => (
            <StaggerItem key={l.id}>
              <Link to={`/app/lecture/${l.id}`}
                className="group rounded-lg bg-surface p-5 hover:bg-primary-light/40 hover:-translate-y-0.5 transition-all duration-300 relative flex flex-col h-full">
